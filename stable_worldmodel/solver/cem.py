...
class CEMSolver:
    ...
    def project_candidates(self, candidates: torch.Tensor) -> torch.Tensor:
        """Project samples into a solver-specific feasible action set.

        The default leaves the historical CEM behavior unchanged.  Planning
        adapters can override this hook when their model-space action
        coordinates have known feasibility bounds.
        """
        return candidates

    def project_mean(self, mean: torch.Tensor) -> torch.Tensor:
        """Project updated CEM parameters before the next sampling round."""
        return mean
    ...
    def solve(
        ...
        for start_idx in range(0, total_envs, self.batch_size):
            ...
            for step in range(self.n_steps):
                ...
                prev_var = batch_var
                batch_mean = self.project_mean(topk_candidates.mean(dim=1))
