...
class GoalDataset:
    ...
    def __init__(
        ...
        seed: int | None = None,
        return_goal_metadata: bool = False,
        episode_end_goal: bool = False,
    ):
        ...
        self.return_goal_metadata = bool(return_goal_metadata)
        self.episode_end_goal = bool(episode_end_goal)
    ...
    def __getitem__(self, idx: int):
        ...
        goal_kind = (
            'episode_end'
            if self.episode_end_goal
            else self._sample_goal_kind()
        )
        if goal_kind == 'episode_end':
            goal_ep_idx = ep_idx
            goal_local_idx = int(self.episode_lengths[ep_idx]) - 1
        elif goal_kind == 'random':
        ...
        if self.return_goal_metadata:
            frameskip = self.dataset.frameskip
            current_end = (
                local_start + (self.current_goal_offset - 1) * frameskip
            )
            goal_offset_steps = (
                goal_local_idx - current_end
                if goal_ep_idx == ep_idx
                else -1
            )
            steps['goal_offset_steps'] = torch.tensor(
                goal_offset_steps, dtype=torch.float32
            )
